...
cat Dockerfile |\
 ...
 grep -i " as " |\
 awk '{print $NF}' |\
 xargs -n 1 -I{} bash -c "(docker build --build-arg DOCKER_REGISTRY=${DOCKER_REGISTRY}/jx $CACHE_FROM -f Dockerfile --target {} -t $DOCKER_REGISTRY/$DOCKER_REGISTRY_ORG/$APP_NAME:stage_{} . && docker push $DOCKER_REGISTRY/$DOCKER_REGISTRY_ORG/$APP_NAME:stage_{}) || exit 255" # Fails if any stage fails.
